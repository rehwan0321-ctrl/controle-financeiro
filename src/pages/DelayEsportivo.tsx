    ...
    try {
      ...
      if (type === "deposito") {
        ...
        if (isDevolucao) {
          ...
          const lucroBruto = isGlauberClient ? valor - cliente.depositos : (valor - custo) - cliente.depositos;
          const lucroAposSplit = (transDividirLucro && lucroBruto > 0) ? lucroBruto / 2 : lucroBruto;
          lucroFinal = isGlauberClient ? lucroAposSplit - custo : lucroAposSplit;
            ...
            <CardContent className="p-4">
              ...
              <div className="mt-2 pt-2 border-t border-border/50 space-y-0.5">
                ...
                {clientes.filter(c => c.depositos > 0 && c.saques === 0 && c.status !== "saque_pendente" && (c.deposito_pendente ?? 0) <= 0 && c.status !== "system").map(c => {
                  const operadorNick = shareLinks.find(l => l.id === c.operator_link_id)?.nick;
                  const infoExtra = [c.fornecedor, operadorNick].filter(Boolean).join(" · ");
                  return (
                    <div key={c.id} className="flex justify-between items-center text-[10px] gap-1">
                      <div className="flex items-center gap-1 min-w-0 overflow-hidden">
                        ...
                        {infoExtra && (
                          <span className="text-[8px] text-muted-foreground/50 truncate italic">{infoExtra}</span>
                        )}
                      </div>
            ...
            {transDialog?.type === "saque" && (
              <>
                ...
                {(() => {
                  ...
                  const isGlauber = transDialog?.cliente.operator_link_id === 'f42962e0-2355-4546-8a62-27207117e34a';
                  const lucroBruto = isGlauber ? val - deposito : (val - cst) - deposito;
