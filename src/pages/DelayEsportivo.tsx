...
const DelayEsportivo = () => {
  ...
  const filtered = useMemo(() => {
    let result = clientes.filter(c => {
      ...
      const isDevolvido = c.status === "devolvido" || (c.saques > 0 && Math.abs(c.saques - c.depositos) < 0.01 && Math.abs(c.lucro ?? 0) < 0.01);
      const isOperando = !isDevolvido && c.status !== "concluido" &&
        ((c.depositos > 0 && c.saques === 0 && c.status !== "saque_pendente") || c.status === "operando");
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="p-4 sm:p-6 space-y-4 sm:space-y-6">
        ...
        {(() => {
          ...
          const operandoCount = allVisible.filter(c =>
            !isDevolvidoFn(c) &&
            c.status !== "concluido" &&
            ((c.depositos > 0 && c.saques === 0 && c.status !== "saque_pendente") || c.status === "operando")
          ).length;
